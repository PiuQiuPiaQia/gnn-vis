...
# ============================================================================
# 有效感受野分析（Effective Receptive Field Analysis）
# ============================================================================
...
try:
    import receptive_field_analysis
    from receptive_field_analysis import (
        ReceptiveFieldConfig,
        analyze_receptive_field,
        calculate_physical_influence_range
    )
    print("感受野分析模块导入成功！")
except Exception as e:
    print(f"导入失败: {e}")
    print("请确保 receptive_field_analysis.py 文件在当前目录")
...
# 创建配置
receptive_field_config = ReceptiveFieldConfig(
    # 目标点：选择一个关注的地理位置
    target_lat=40.0,   # 北纬40度（例如：北京、纽约、马德里）
    target_lon=116.0,  # 东经116度

    # 变量配置
    input_variable='2m_temperature',   # 扰动输入的哪个变量
    output_variable='2m_temperature',  # 观察输出的哪个变量

    # 扰动配置
    perturbation_radius_degrees=[
        1, 2, 3, 5, 7, 10, 15, 20, 30, 40, 50
    ],  # 扫描的半径范围（度）
    # 对应约: 111km, 222km, 333km, 555km, ..., 5550km

    perturbation_magnitude=1.0,  # 扰动幅度（标准差的倍数）
    n_perturbations=30,          # 每个半径的扰动次数（建议20-50）
)

print("配置完成！")
print(f"目标点: ({receptive_field_config.target_lat}°N, {receptive_field_config.target_lon}°E)")
print(f"将测试 {len(receptive_field_config.perturbation_radius_degrees)} 个不同半径")
print(f"总计需要运行 {len(receptive_field_config.perturbation_radius_degrees) * receptive_field_config.n_perturbations} 次模型推理")
print(f"预计耗时: 约 {len(receptive_field_config.perturbation_radius_degrees) * receptive_field_config.n_perturbations * 0.5 / 60:.1f} 分钟")


# In[ ]:

# @title 步骤1: 计算物理过程的影响范围（理论值）

print("="*80)
print("首先计算各物理过程的理论影响范围")
print("="*80)

physical_ranges = calculate_physical_influence_range(
    eval_inputs,
    receptive_field_config,
    time_horizon=6*3600  # 6小时预报
)
...
processes = list(physical_ranges.keys())
distances = [physical_ranges[p] for p in processes]
colors_palette = ['#E63946', '#F77F00', '#06D6A0', '#118AB2', '#073B4C']

bars = ax.barh(processes, distances, color=colors_palette, alpha=0.7)

# 添加数值标签
for i, (bar, dist) in enumerate(zip(bars, distances)):
    ax.text(dist + 50, i, f'{dist:.0f} km',
            va='center', fontweight='bold', fontsize=11)

ax.set_xlabel('影响半径 (km)', fontsize=13, fontweight='bold')
ax.set_title('不同物理过程的6小时影响范围', fontsize=15, fontweight='bold')
...
# @title 步骤2: 运行感受野分析（输入扰动法）

print("="*80)
print("开始运行 GraphCast 有效感受野分析")
print("="*80)
print("这将需要一些时间，请耐心等待...")
print()

# 运行分析
receptive_field_results = analyze_receptive_field(
    run_forward_jitted=run_forward_jitted,
    eval_inputs=eval_inputs,
    eval_targets=eval_targets,
    eval_forcings=eval_forcings,
    config=receptive_field_config,
    method='perturbation',  # 使用输入扰动法
    save_results=True       # 保存结果到文件
)

print("\n分析完成！")
print("结果已保存到:")
print("  - receptive_field_analysis.png  (感受野曲线图)")
print("  - receptive_field_results.pkl   (原始数据)")
...
# 提取结果
perturbation_results = receptive_field_results['perturbation_results']
physical_ranges = receptive_field_results['physical_ranges']

# 计算关键指标
radii_km = [r * 111 for r in sorted(perturbation_results.keys())]
mean_changes = [np.mean(perturbation_results[r]) for r in sorted(perturbation_results.keys())]

# 1. 找到有效感受野半径（80%饱和点）
max_change = max(mean_changes)
threshold_80 = 0.8 * max_change
threshold_50 = 0.5 * max_change

erf_80 = None
erf_50 = None

for r_km, m in zip(radii_km, mean_changes):
    if erf_50 is None and m >= threshold_50:
        erf_50 = r_km
    if erf_80 is None and m >= threshold_80:
        erf_80 = r_km
        break
...
if erf_50:
    print(f"\n50% 饱和半径: {erf_50:.0f} km ({erf_50/111:.1f}°)")
if erf_80:
    print(f"80% 饱和半径: {erf_80:.0f} km ({erf_80/111:.1f}°)")

# 2. 与物理过程对比
print("\n与物理过程对比:")
print("-" * 60)

advection_range = physical_ranges['平流过程 (Advection)']
print(f"平流过程影响范围:    {advection_range:.0f} km")

if erf_80:
    ratio = erf_80 / advection_range
    print(f"GraphCast ERF:        {erf_80:.0f} km")
    print(f"ERF / 平流范围:       {ratio:.2f}x")

    print("\n解释:")
    if ratio > 2:
        print("  ✓ GraphCast 的有效感受野远大于平流过程！")
        print("  ✓ 模型可能捕捉了:")
        print("    - 大尺度环流模式")
        print("    - 波动传播（重力波、罗斯贝波）")
        print("    - 远程相关（teleconnection）")
        print("  ✓ 这解释了为什么 GraphCast 能超越简化的平流方程")
    elif ratio > 1:
        print("  ✓ GraphCast 的感受野略大于平流过程")
        print("  ✓ 模型额外利用了一些非局地信息")
    elif ratio > 0.5:
        print("  ⚠ GraphCast 的感受野接近平流过程")
        print("  ⚠ 模型主要依赖局地信息")
    else:
        print("  ⚠ GraphCast 的感受野小于平流过程")
        print("  ⚠ 可能的原因:")
        print("    - 模型架构限制（GNN层数、mesh分辨率）")
        print("    - 训练数据不足以学习远程依赖")
...
for i in range(min(5, len(radii_km))):
    r = radii_km[i]
    change = mean_changes[i]
    pct = (change / max_change) * 100
    print(f"  {r:6.0f} km: 输出变化 {change:.6f} ({pct:5.1f}% of max)")


# In[ ]:

# @title 步骤4: 空间敏感性可视化（可选 - 需要完整数据）

# 这部分需要保存每次扰动的空间分布，会占用较多内存
# 可以通过修改 receptive_field_analysis.py 实现

print("空间敏感性可视化需要修改代码以保存完整的扰动结果")
print("当前版本仅计算目标点的变化，以节省内存")
...
quick_radii = [1, 5, 10, 20, 40]  # 只测试5个半径
quick_n_pert = 10  # 每个半径10次扰动

multi_var_results = {}

for var in variables_to_test:
    if var not in eval_inputs:
        print(f"跳过 {var} (不在输入数据中)")
        continue

    print(f"\n测试变量: {var}")

    quick_config = ReceptiveFieldConfig(
        target_lat=receptive_field_config.target_lat,
        target_lon=receptive_field_config.target_lon,
        input_variable=var,
        output_variable=var if var in eval_targets else '2m_temperature',
        perturbation_radius_degrees=quick_radii,
        n_perturbations=quick_n_pert
    )

    var_results = receptive_field_analysis.compute_receptive_field_perturbation(
        run_forward_jitted,
        eval_inputs,
        eval_targets,
        eval_forcings,
    ...
    multi_var_results[var] = var_results

# 绘制对比图
if multi_var_results:
    fig, ax = plt.subplots(1, 1, figsize=(12, 7))

    colors_var = ['#E63946', '#F77F00', '#06D6A0']

    for i, (var, results) in enumerate(multi_var_results.items()):
        radii = sorted(results.keys())
        means = [np.mean(results[r]) for r in radii]
        radii_km = [r * 111 for r in radii]

        ax.plot(radii_km, means, 'o-', linewidth=2, markersize=8,
                label=var, color=colors_var[i % len(colors_var)])

    ax.set_xlabel('扰动半径 (km)', fontsize=13, fontweight='bold')
    ax.set_ylabel('输出变化', fontsize=13, fontweight='bold')
    ax.set_title('不同变量的有效感受野对比', fontsize=15, fontweight='bold')
    ...
    print("\n观察:")
    print("- 不同变量的感受野可能不同")
    print("- 温度通常有较大的感受野（受平流影响）")
    print("- 湿度可能有较小的感受野（更局地）")
...
print("""
本次有效感受野分析揭示了 GraphCast 如何利用输入信息：
...
1. 【空间尺度】
   ...
   说明: GraphCast 能够捕捉 [局地/中尺度/大尺度] 的气象模式

2. 【与物理公式对比的公平性】
   现在我们知道:
   - 简化物理公式（如平流方程）只考虑局地信息
   - GraphCast 利用了更大范围的空间信息
   - 这解释了为什么 GraphCast 预报更准确

   公平的对比应该是:
   ✓ 给物理公式提供同样范围内的信息（区域平均等）
   ✓ 或者限制 GraphCast 的感受野到物理过程范围
...
4. 【改进方向】
   - 如果感受野过小 → 增加 GNN 层数、mesh 分辨率
   - 如果感受野过大 → 可能存在过拟合，需要正则化
   - 可以设计"物理约束的感受野"训练策略

5. 【论文写作角度】
   标题示例:
   "Understanding GraphCast Through Effective Receptive Field Analysis:
    Why Data-Driven Models Outperform Simplified Physics"

   核心贡献:
   - 量化了深度学习气象模型的有效感受野
   - 揭示了模型超越简化物理方程的原因（利用了更大尺度信息）
   - 提出了公平对比深度学习与物理模型的框架
""")

print("\n下一步建议:")
print("1. 测试不同地理位置的感受野（高纬度 vs 低纬度）")
print("2. 测试不同预报时长的感受野变化")
print("3. 分析感受野的各向异性（东西方向 vs 南北方向）")
print("4. 设计'物理感受野约束'的训练方法")
